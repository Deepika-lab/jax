...
def stop_trace() -> ProfileData:
  ...
  if log_dir is not None:
    # Export via session rather than profile_data so that
    # session.profiler_options.session_id is preserved instead of dropped.
...
def trace(
  ...
  try:
    ...
    # Guard stop() so calling stop_trace() or session.stop() inside
    # the trace context does not raise RuntimeError("No profile started") on exit.
    if session.is_running:
      session.stop()
    if log_dir is not None and session.profile_data is not None:
      session.export_profile_data(
          log_dir,
          create_perfetto_link=create_perfetto_link,
          create_perfetto_trace=create_perfetto_trace,
